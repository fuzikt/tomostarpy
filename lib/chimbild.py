...
class bildPolygonItem:
    def __init__(self, polygonVertices, value=0, shiftX=0, shiftY=0, shiftZ=0, r=1, g=1, b=0):
...
class bildPolygonData:

    def __init__(self):
        self.bildPolygonItems = []

    def addItem(self, polygonVertices, value=0, shiftX=0, shiftY=0, shiftZ=0, r=1, g=1, b=0):
        self.bildPolygonItems.append(bildPolygonItem(polygonVertices, value, shiftX, shiftY, shiftZ, r, g, b))
    ...
    def createColorGradient(self):
        ...
        # get the min and max value (ie range) of the desired label in the star file
        minColor = self.bildPolygonItems[0].value
        maxColor = self.bildPolygonItems[0].value
        for item in self.bildPolygonItems:
            minColor = min(minColor, item.value)
            maxColor = max(maxColor, item.value)
        ...
        for item in self.bildPolygonItems:
            if minColor != maxColor:
                ...
                coloringArrayElement = [1, 1, 0]
            self.bildPolygonItems[self.bildPolygonItems.index(item)].r = coloringArrayElement[0]
            self.bildPolygonItems[self.bildPolygonItems.index(item)].g = coloringArrayElement[1]
            self.bildPolygonItems[self.bildPolygonItems.index(item)].b = coloringArrayElement[2]

    def writeBuildPolygonFile(self, outputBuildPolygonFile):
        self.createColorGradient()

        with open(outputBuildPolygonFile, 'w') as bildPolygonFile:
            for item in self.bildPolygonItems:
                bildPolygonFile.write(".color %0.3f %0.3f %0.3f\n" % (item.r, item.g, item.b))
                bildPolygonFile.write(".polygon ")
                for vertex in item.polygonVertices:
                    bildPolygonFile.write("%0.3f %0.3f %0.3f " % (vertex[0]+item.shiftX,vertex[1]+item.shiftY,vertex[2]+item.shiftZ))
                bildPolygonFile.write("\n")